...
export default function ConquistasScreen() {
  ...
  // agrupa o histórico por categoria para o mini relatório
  const acoesPorCategoria = historico.reduce((acc, item) => {
    acc[item.acao] = (acc[item.acao] || 0) + 1;
    return acc;
  }, {});

  const topAcoes = Object.entries(acoesPorCategoria)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 3);
        ...
        return (
          ...
          >
            ...
            <View style={styles.conquistaInfo}>
              ...
              {!ganhou && (
                <View style={styles.progressoConquista}>
                  <Text style={styles.progressoConquistaTexto}>
                    Meta: {conquista.objetivo} {conquista.tipo === 'max_streak' ? 'dias de streak' : 'ações'}
                  </Text>
                </View>
              )}
            </View>
        ...
        <>
          ...
          {historico.slice(0, 10).map(item => (
            <View key={item.id} style={styles.historicoItem}>
              ...
              <Text style={styles.historicoPontos}>+{item.pontos} pts</Text>
            </View>
          ))}
        </>
      )}
